...
export default function SimulatorPanel({ result, isLoading }: SimulatorPanelProps) {
  if (isLoading) {
    return (
          ...
          {sim.effects.map((effect, i) => (
            <div key={i} className="flex items-center gap-4 p-4 rounded-xl"
              ...
              <div>
                ...
                <p className="text-xs uppercase tracking-wider mt-1 font-semibold" style={{ color: 'var(--text-muted)' }}>
            ...
            {sim.balanceChanges.map((change, i) => (
              <div key={i} className="flex items-center justify-between p-3.5"
                style={{
                  ...
                  borderBottom: i < sim.balanceChanges.length - 1 ? '1px solid var(--border-subtle)' : 'none',
                }}>
        ...
        <div className="space-y-2">
          ...
          {sim.approvalChanges.map((approval, i) => (
            <div key={i} className="p-4 rounded-xl" style={{
              ...
              border: `1px solid ${approval.amount === 'UNLIMITED' ? 'rgba(239,68,68,0.15)' : 'var(--border-subtle)'}`,
            }}>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs font-extrabold px-3 py-1 rounded-full" style={{
                  background: approval.amount === 'UNLIMITED' ? 'rgba(239,68,68,0.2)' : 'rgba(59,130,246,0.12)',
                  color: approval.amount === 'UNLIMITED' ? '#f87171' : 'var(--accent-blue)',
                }}>
